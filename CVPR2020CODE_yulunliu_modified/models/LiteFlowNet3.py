...
def flow(tenFirst, tenSecond):
    global flow_model

    if flow_model is None:
        flow_model = Network().cuda().eval()
    # end
    ...
    tenFlow = torch.nn.functional.interpolate(input=flow_model(tenPreprocessedFirst, tenPreprocessedSecond), size=(intHeight, intWidth), mode='bilinear', align_corners=False)
