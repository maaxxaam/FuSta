...
class UPFlow_net(tools.abstract_model):
    class config(tools.abstract_config):
        def __init__(self):
            ...
            self.if_sgu_upsample = True  # if use sgu upsampling
            self.if_use_cor_pytorch = True  # use my implementation of correlation layer by pytorch. only for test model in cpu(corr layer cuda is not compiled)
    ...
    def demo(cls):
        ...
        net = net_conf()  # .cuda()
        net.eval()'''
        net = UPFlow_net.config().cuda().eval()
